...
class Main_Hub : 
	...
	def update_price_labels(self):
		...
		for company in self.companies:
			for share_name, share_data in company.items():	
				if share_name in self.price_labels:
					current_price = f"{share_data['shares_price']}$"
		...
		for company in companies:
			for share_name, share_data in company.items():	
				if share_name in self.price_labels:
					...
					price_label = self.stock_canvas.create_text(190, y_offset, text = f"{share_data['shares_price']}$", anchor ='e')
